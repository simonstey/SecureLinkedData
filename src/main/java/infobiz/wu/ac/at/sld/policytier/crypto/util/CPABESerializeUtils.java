...
//		pub.p = PairingFactory.getPairing(params);
	...
	public static byte[] bswabeCphSerialize(BswabeCph cph) {
		ArrayList<Byte> arrlist = new ArrayList<Byte>();
		CPABESerializeUtils.serializeElement(arrlist, cph.cs);
		CPABESerializeUtils.serializeElement(arrlist, cph.c);
		CPABESerializeUtils.serializePolicy(arrlist, cph.p);
	...
	public static BswabeCph bswabeCphUnserialize(BswabePub pub, byte[] cphBuf) {
		...
		offset = CPABESerializeUtils.unserializeElement(cphBuf, offset, cph.cs);
		offset = CPABESerializeUtils.unserializeElement(cphBuf, offset, cph.c);

		offset_arr[0] = offset;
		cph.p = CPABESerializeUtils.unserializePolicy(pub, cphBuf, offset_arr);
		offset = offset_arr[0];
